...
this.reply(m.chat, `🐈 𝙉𝙊 𝙏𝙄𝙀𝙉𝙀 𝘿𝘼𝙍𝙄𝙀𝙇𝘾𝙊𝙄𝙉𝙎`, m)
continue     
}
...
if (m.money)
m.reply(+m.money + ' 𝘿𝘼𝙍𝙄𝙀𝙇𝘾𝙊𝙄𝙉𝙎 🐱 𝙐𝙎𝘼𝘿𝙊(𝙎)')  
break
}}} catch (e) {
console.error(e)
} finally {
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '😻 𝗦𝘂𝗽𝗲𝗿 𝘿𝙖𝙧𝙞𝙚𝙡𝘽𝙤𝙩-𝙈𝘿 😻') :
